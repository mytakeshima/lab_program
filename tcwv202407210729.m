...
info = ncinfo('202407210729tcwv.nc');

% NetCDFファイルからデータを読み込む
data = ncread('202407210729tcwv.nc', 'tcwv');

lat = ncread('202407210729tcwv.nc', 'latitude'); % 緯度データを読み込む
lon = ncread('202407210729tcwv.nc', 'longitude'); % 経度データを読み込む
...
for time_index = 1:216
    ...
    xlabel('経度');
    ylabel('緯度');
    title(['日時: ' datestr(current_time, 'yyyy/mm/dd HH:MM')]);


    % カラーバーにラベルを追加
    ylabel(colorbarHandle, '総水蒸気量 kg\m^2'); % カラーバーのラベルを設定
    ...
    % プロットの保存や表示に関する処理
    saveas(gcf, fullfile('C:\Users\murqk\Desktop\plot\07210729tcwv\', ['plot_' num2str(time_index) '.png']));
