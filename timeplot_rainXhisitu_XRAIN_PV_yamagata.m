% 比湿データの読み込みと平均計算
q_data = ncread('202407210729UTmodel_cloud.nc', 'q'); % 比湿データ
lat_q = ncread('202407210729UTmodel_cloud.nc', 'latitude');
lon_q = ncread('202407210729UTmodel_cloud.nc', 'longitude');

% 圧力レベルを選択（1: 500 hPa, 2: 850 hPa, 3: 1000 hPa）
pressure_level_idx = 2; % 例: 850 hPaを選択
...
% 比湿データの平均値を計算（選択した圧力レベル）
q_selected = q_data(lon_idx_q, lat_idx_q, pressure_level_idx, :);
...
num_time_steps = size(q_selected, 4); % 時間ステップ数
...
% プロットの保存
saveas(gcf, fullfile('C:\Users\murqk\Desktop\plot\2024山形線状降水帯', '比湿_降水量時系列プロット.png'));
